...
function MyProfilePage() {
  ...
  useEffect(() => {
    if (data === null) {
      axios
        .get(`http://localhost:8080/profile/myProfile?UserID=${userID}`)
        .then((response) => {
  ...
  const handleFollowClick = async () => {
    try {
      ...
      await axios.post(
        `http://localhost:8080/follow?userID=${userID}&profileID=${userID}`
      );
      fetchFollowStatus();
    } catch (error) {
      ...
      <div className="profileheader">
        {loading ? (
          ...
          <div className="profilehorizontalcontainer">
            ...
            <h1>{data.userName}</h1>
            <MyFollowers
              place={`Followers: ${data.followersCount}`}
              myID={userID}
            />
            <MyFollowering
              place={`Following: ${data.followingCount}`}
              myID={userID}
            />
            <button
              className="follow"
