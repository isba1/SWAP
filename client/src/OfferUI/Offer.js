...
const Offer = ({ SellerPost, myID }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await axios.get(
        `http://localhost:8080/offer/itemsToOffer?userID=${myID}`
      );
  ...
  return (
    <div>
      <button className="makeofferbutton" onClick={toggleModal}>
        Make Offer
      </button>
        ...
        <div className="offermodal">
          ...
          <div className="offermodal-content">
            ...
            <NameList
              ...
              myid={myID}
            />
            <button className="offerclose-modal" onClick={toggleModal}>
              X
            </button>
          </div>
        </div>
